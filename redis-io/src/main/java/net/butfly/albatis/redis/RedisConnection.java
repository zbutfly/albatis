...
public class RedisConnection extends NoSqlConnection<Jedis> {
	...
	public RedisConnection(URISpec uriSpec) {
		...
		jedis = new Jedis(isas[0].getHostString(), isas[0].getPort());
	...
	public RedisListInput input(TableDesc... table) throws IOException {
		...
		RedisListInput i = new RedisListInput("RedisListInput", this, l.toArray(new String[0]));
		return i;
	}
...
//		List<HashMap> collect = result.stream().map(r -> JsonSerder.JSON_MAPPER.der(r, HashMap.class)).collect(Collectors.toList());
//		System.out.println(collect.toString());
//		System.out.println(jedis.llen("AFTER_VEHICLE"));
