...
public class MongodbInput extends InputQueueImpl<DBObject> {
	...
	public MongodbInput(final String name, String uri, final String table, int inputBatchSize, final String... filter) throws IOException {
		super(name);
		logger.info("MongoDB input [" + name + "] from " + uri);
		this.mdb = new MongoDB(uri);
		DBCursor c;
		if (null == filter || filter.length == 0) c = mdb.connect().getCollection(table).find();
		else if (filter.length == 1) {
			Map<String, Object> m = JsonSerder.JSON_MAPPER.der(filter[0]);
			c = mdb.connect().getCollection(table).find(new BasicDBObject(m));
		} else {
			...
			and.put("$and", filters);
			c = mdb.connect().getCollection(table).find(and);
		}
		c.batchSize(inputBatchSize).addOption(Bytes.QUERYOPTION_NOTIMEOUT);
		this.cursor = c;
	}
