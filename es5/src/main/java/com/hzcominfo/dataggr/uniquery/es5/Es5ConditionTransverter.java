...
public interface Es5ConditionTransverter extends ConditionTransverter {
	...
	static QueryBuilder of(JsonObject json) {
		if (null == json || 0 == json.size())
			return new UnrecognizedEs5ConditionTransverter().toEs5Query();
		JsonElement element;
		...
		for (String key : json.keySet()) {
			...
			case "or":
				...
				return "and".equals(key) ? new AndEs5ConditionTransverter(of(jl), of(jr)).toEs5Query()
						: new OrEs5ConditionTransverter(of(jl), of(jr)).toEs5Query();
			case "is_null":
			case "is_not_null":
				field = json.get(key).getAsString();
				return "is_null".equals(key) ? new IsNullEs5ConditionTransverter(field).toEs5Query()
						: new IsNotNullEs5ConditionTransverter(field).toEs5Query();
			case "equals":
			case "not_equals":
			case "greater_than":
			...
			case "not_between":
				...
				return "between".equals(key)
						? new BetweenEs5ConditionTransverter(field, se.get(0), se.get(1)).toEs5Query()
			...
			case "geo_distance":
				...
				return new GeoDistanceEs5ConditionTransverter(field, params).toEs5Query();
			case "geo_box":
				object = json.getAsJsonObject(key);
				field = new ArrayList<>(object.keySet()).get(0);
				params = new ArrayList<>();
				object.get(field).getAsJsonArray().forEach(e -> params.add(e.getAsBigDecimal().doubleValue()));
				return new GeoBoxEs5ConditionTransverter(field, params).toEs5Query();
			case "geo_polygon":
				object = json.getAsJsonObject(key);
				field = new ArrayList<>(object.keySet()).get(0);
				params = new ArrayList<>();
	...
	class GeoDistanceEs5ConditionTransverter implements Es5ConditionTransverter {
		private String field;
		private List<Double> params;

		public GeoDistanceEs5ConditionTransverter(String field, List<Double> params) {
			this.field = field;
			this.params = params;
			if (params.size() != 3)
		...
		public QueryBuilder toEs5Query() {
			return QueryBuilders.geoDistanceQuery(field).point(params.get(0), params.get(1))
					.distance(params.get(2), DistanceUnit.KILOMETERS).geoDistance(GeoDistance.PLANE);
		}
	}

	// geo_box(field,top,left,bottom,right)
	class GeoBoxEs5ConditionTransverter implements Es5ConditionTransverter {
		private String field;
		private List<Double> params;

		public GeoBoxEs5ConditionTransverter(String field, List<Double> params) {
			this.field = field;
			this.params = params;
			if (params.size() != 4)
	...
	class GeoPolygonEs5ConditionTransverter implements Es5ConditionTransverter {
		private String field;
		private List<Double> params;

		public GeoPolygonEs5ConditionTransverter(String field, List<Double> params) {
			this.field = field;
			this.params = params;
	...
	static boolean isNestedField(String field) {
		if (null == field || field.isEmpty())
			throw new RuntimeException("field can NOT be null or empty!");
	...
	static QueryBuilder nestedQueryWrapper(String field, QueryBuilder query) {
		...
		// says: The score_mode allows to set how inner children matching
	...
	/**
	 * 查询null值 <a
	 * href=https://www.elastic.co/guide/en/elasticsearch/reference/5.5/null-
	 * value.html>null-value</a>
	 */
	...
	class IsNotNullEs5ConditionTransverter implements Es5ConditionTransverter {
		...
		/**
		 * see https://www.elastic.co/guide/en/elasticsearch/reference/5.5/null-
		 * value.html
		 * https://www.elastic.co/guide/en/elasticsearch/reference/5.5/query-dsl
		 * -exists-query.html#_literal_missing_literal_query
		 */
		@Override
		public QueryBuilder toEs5Query() {
	...
	class LikeEs5ConditionTransverter implements Es5ConditionTransverter {
		...
		@Override
		public QueryBuilder toEs5Query() {
			return nestedQueryWrapper(field,
					QueryBuilders.wildcardQuery(field, value.toString().replaceAll("%", "*").replaceAll("_", "?")));
		}
	}
	...
	class NotBetweenEs5ConditionTransverter implements Es5ConditionTransverter {
		...
		public QueryBuilder toEs5Query() {
			return nestedQueryWrapper(field,
					QueryBuilders.boolQuery().mustNot(QueryBuilders.rangeQuery(field).from(start, true).to(end, true)));
		}
	}
