...
public class Es5NestedHavingTest {
	...
	public void init() throws UnknownHostException {
		...
		client = new PreBuiltTransportClient(settings)
				...
				.addTransportAddress(new TransportAddress(InetAddress.getByName("172.30.10.32"), 39300))
				.addTransportAddress(new TransportAddress(InetAddress.getByName("172.30.10.33"), 39300));
	...
	public void h1() {
		...
		requestBuilder.setIndices(index).setTypes(type).setQuery(QueryBuilders.matchAllQuery())
				// .setQuery(QueryBuilders.nestedQuery("LG", QueryBuilders.matchAllQuery(),
				// ScoreMode.Avg).innerHit(InnerHitBuilder.fromXContent("{}")))
				.setSize(0)
				.addAggregation(AggregationBuilders.nested("nested_count", "LG")
	...
	public void q1() {
		 ...
		 * .must(QueryBuilders.rangeQuery("LG.RZSJ_month_s").from("201001").includeLower
		 * (true));
		 */
		...
		requestBuilder.setIndices(index).setTypes(type).setQuery(query).setSize(0)
	...
	@Test
	public void h2() {
		// String sql = "select SFZH_s, XM_s, LG.LGDM_LGMC_FORMAT_s from
		// tdl_uniquery_test.person where MODEL_NAME_s = 'gazhk_LGY_NB' limit 10";
		// String sql = "select SFZH_s, XM_s, LG.LGDM_LGMC_FORMAT_s from
		// tdl_uniquery_test.person where LG.LGDM_LGMC_FORMAT_s = '横塘村十组旅馆' limit 10";
		String sql = "select SFZH_s, XM_s, LG.LGDM_LGMC_FORMAT_s, count(LG.ZKLSH_s) as cnt from " + index + "." + type
				+ " where LG.LGDM_LGMC_FORMAT_s = '横塘村十组旅馆' group by SFZH_s having cnt > 3";
		// String sql = "select SFZH_s, count(LG.LGDM_LGMC_FORMAT_s) as cnt from " +
		// index + "." + type + " where LG.LGDM_LGMC_FORMAT_s = '横塘村十组旅馆' group by
		// SFZH_s having cnt > 3";
		// String sql = "select SFZH_s, count(LG.LGDM_LGMC_FORMAT_s) as cnt from " +
		// index + "." + type + " group by SFZH_s having cnt > 3";
		// String sql = "select SFZH_s, count(SFZH_s) as cnt from " + index + "." + type
		// + " group by SFZH_s having cnt > 3";
	...
	public void h3() {
		...
		Script script = new Script(Script.DEFAULT_SCRIPT_TYPE, "expression", "cnt >= 2 and cnt < 4",
				Collections.emptyMap());
		BucketSelectorPipelineAggregationBuilder builder = PipelineAggregatorBuilders.bucketSelector("having-filter",
				bsPathMap, script);
		...
		srBuilder.addAggregation(AggregationBuilders.terms("name_aggrs").field("name.keyword")
				.subAggregation(AggregationBuilders.count("cnt").field("name.keyword")).subAggregation(builder));
	...
	@Test
	public void h9() {
		// String sql = "select zjhm, count(0) as cnt from edw_lgy_nb where rzrq >=
		// now() - 180 days and xzqh = '3309123' and lgdm_format like '%如家%' group by
		// zjhm having cnt > 3";
		// String sql = "select zjhm, count(0) from edw_lgy_nb where rzrq >= now() - 180
		// days and xzqh = '3309123' and lgdm_format like '%如家%' group by zjhm having
		// count(0) > 3";
		// String sql = "select zjhm, count(0) as cnt from edw_lgy_nb where rzrq >=
		// 1520478480 and xzqh = '3309123' and lgdm_format like '%如家%' group by zjhm
		// having cnt > 3";
