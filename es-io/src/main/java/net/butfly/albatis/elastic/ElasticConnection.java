...
public class ElasticConnection extends DataConnection<TransportClient> implements ElasticConnect {
    ...
    public void construct(Map<String, Object> indexConfig, FieldDesc... fields) {
        if (indexConfig == null || indexConfig.isEmpty())
            throw new RuntimeException("Please at least put index/type into indexConfig!");
        ...
        if (client.admin().indices().prepareExists(index).execute().actionGet().isExists()) {
            ...
            if (!r.isAcknowledged()) logger().error("Mapping failed on type [" + type + "]" + req.toString());
                ...
                logger().info(() -> "Mapping on [" + type + "] construced sussesfully: \n\t" + JsonSerder.JSON_MAPPER.ser(mapping));
            return;
        }
        ...
        if (client.admin().indices().exists(existsRequest).actionGet().isExists()) {
            response = client.admin().indices().prepareAliases().addAlias(index, alias).execute().actionGet();
            if (!response.isAcknowledged())
                logger().error("create elastic index alias failure:" + response.toString());
            else
                logger().info("create elastic index alias successful");
        }
    }
    ...
    public void construct(Map<String, Object> mapping, String... types) {
        ...
        for (String t : tps) {
            ...
            if (!r.isAcknowledged()) logger().error("Mapping failed on type [" + t + "]" + req.toString());
                ...
                logger().info(() -> "Mapping on [" + t + "] construced sussesfully: \n\t" + JsonSerder.JSON_MAPPER.ser(mapping));
        }
    }
}
