...
public class HbaseConnection extends DataConnection<org.apache.hadoop.hbase.client.Connection> implements IOFactory {
    ...
    public void construct(String dbName, String table, TableDesc tableDesc, List<FieldDesc> fields) {
        try {
            ...
            createHbaseTable(client, table, startKey, endKey, splitKey, region, families);
        } catch (IOException e) {
            throw new RuntimeException("create hbase table failure   ", e);
        }
    }

    public static void createHbaseTable(Connection client, String tableName, Object startKey, Object endKey, Object splitKey, Object numRegions, Object families) throws IOException {
        byte[] startKeys = null;
        byte[] endKeys = null;
        byte[][] splitKeys = null;
        if (null != startKey)
        ...
        if (families instanceof Map) {
            ...
            familyMap.forEach((k, v) -> {
                ...
                if (null != familyParam.get("keep_deleted_cells")) {
                    KeepDeletedCells keepDeletedCells = KeepDeletedCells.valueOf(familyParam.get("keep_deleted_cells").toString().toUpperCase());
                    columnFamily.setKeepDeletedCells(keepDeletedCells);
                }
                if (null != familyParam.get("data_block_encoding")) {
                    DataBlockEncoding dataBlockEncoding = DataBlockEncoding.valueOf(familyParam.get("data_block_encoding").toString().toUpperCase());
        ...
        if (null != splitKey)
            ...
            } else if ((Integer) numRegions == 3)
                admin.createTable(table, new byte[][]{startKeys, endKeys});
            else
