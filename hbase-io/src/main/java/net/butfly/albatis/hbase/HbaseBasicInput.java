 ...
 * reader from hbase.
 */
public abstract class HbaseBasicInput<M extends Message> extends Namedly implements Input<M> {
	private final long SCAN_BYTES = Props.propL(HbaseBasicInput.class, "scan.bytes", 3145728); // 3M
	private final int SCAN_ROWS = Props.propI(HbaseBasicInput.class, "scan.rows", 1);
	...
	public HbaseBasicInput(String name, HbaseConnection conn) {
		super(name);
		hconn = conn;
	...
	@Override
	public void dequeue(Consumer<Sdream<M>> using) {
		TableScaner s;
		while (opened() && !empty())
	...
	protected abstract Sdream<M> m(Sdream<Message> m);;
}
