...
public abstract class FilterLoader {
				...
				if (configList == null || configList.size() == 0) {
					try {
						...
						if (System.getProperty("search.filter.config") != null) {
							...
							if (!path.contains(".xml")) { throw new SearchAPIException(""); }
						} else {
	...
	protected static List<FilterChainConfig> XMLParser(final String path) throws SearchAPIException {
		if (path == null || path.isEmpty()) { throw new SearchAPIException("NULL PATH"); }
		...
		try {
			...
			for (Element element : chain) {
				...
				if (element.getChildren("param") != null && element.getChildren("param").size() > 0) {
					chainConfig.params = new HashMap<>();
					for (Element param : element.getChildren("param")) {
						chainConfig.params.put(param.getAttributeValue("key"), param.getAttributeValue("value"));
					}
				}
				...
				for (Element el : filters) {
					...
					if (el.getChildren("param") != null && element.getChildren("param").size() > 0) {
						config.params = new HashMap<>();
						for (Element param : element.getChildren("param")) {
							config.params.put(param.getAttributeValue("key"), param.getAttributeValue("value"));
						}
					}
	...
	/**
	 * 获取执行invoke方法的类名， 考虑到调用invoker的类可能会被大量其他的类逐步调用，所以从最底部逐渐判断，直到第一个不为本类的名称为止
	 *
	 * @return 类的全名
	 */
	private static String getInvokedClazz() {
		...
		for (StackTraceElement element : e.getStackTrace()) {
			if (!element.getClassName().equals(FilterLoader.class.getCanonicalName())) { return element.getClassName(); }
		}
		return null;
	}
	...
	private FilterChainConfig findByName(String name, List<FilterChainConfig> source) {
		for (FilterChainConfig config : source) {
			if (config.name != null && config.name.equals(name)) { return config; }
		}
		return null;
	}
	...
	private FilterChainConfig findByInvoke(String invoke, List<FilterChainConfig> source) {
		for (FilterChainConfig config : source) {
			if (config.invoke != null && config.invoke.equals(invoke)) { return config; }
		}
		return null;
	}
