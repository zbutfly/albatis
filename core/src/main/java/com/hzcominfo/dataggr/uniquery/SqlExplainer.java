...
public class SqlExplainer {
	...
	public static SqlParser.ConfigBuilder DEFAULT_PARSER_CONFIG = SqlParser.configBuilder()
			...
			.setConformance(SqlConformanceEnum.DEFAULT).setConformance(SqlConformanceEnum.LENIENT) // allow everything
			// .setConformance(SqlConformanceEnum.MYSQL_5) // 默认sql标准为mysql_5 allow Limit
			// start,count ,,,start is from 1
	...
	private static void analysisFields(List<SqlNode> selectList, JsonObject json) {
		...
		for (SqlNode n : selectList) {
			...
			case AS:
				...
				String names = SqlDynamicParam.class.isInstance(nd) ? newDynamicParamMark(nd)
						: fieldNameFromSqlNode(nd);
	...
	private static void analysisConditionExpression(SqlNode condition, JsonObject tree) {
		...
		case MIN:
			break;
		case OTHER_FUNCTION:
			analysisFuncConditionExpression(sc, tree);
			break;
		default:
	...
	private static void analysisFuncConditionExpression(SqlCall sc, JsonObject tree) {
		String func = sc.getOperator().getName().toLowerCase();
		if ("query_all".equals(func))
			analysisQueryAllConditionExpression(sc, tree);
		else if (func.startsWith("geo"))
			analysisGeoConditionExpression(sc, tree);
	}

	private static void analysisQueryAllConditionExpression(SqlCall sc, JsonObject tree) {
		String func = sc.getOperator().getName().toLowerCase();
		SqlNode node0 = sc.operand(0);
		String v = getNodeName(node0);
		tree.addProperty(func, v);
	}

	private static void analysisGeoConditionExpression(SqlCall sc, JsonObject tree) {
		String func = sc.getOperator().getName().toLowerCase();
		JsonObject json = new JsonObject();
		int operandCount = sc.operandCount();
		SqlNode node0 = sc.operand(0);
		String sfield = getNodeName(node0);
		JsonArray array = new JsonArray();
		for (int i = 1; i < operandCount; i++) {
			SqlNode node = sc.operand(i);
			switch (node.getKind()) {
			case IDENTIFIER:
				array.add(((SqlIdentifier) node).toString());
				break;
			case LITERAL:
				array.add(((SqlLiteral) node).bigDecimalValue());
				break;
			case DYNAMIC_PARAM:
	...
	private static void analysisUnaryConditionExpression(SqlCall sc, JsonObject object) {
		...
		if (node instanceof SqlDynamicParam)
			value = newDynamicParamMark(node);
		else
	...
	private static void analysisMultipleConditionExpression(SqlCall sc, JsonObject json) {
		...
		String field;
		if (sc.operand(0).getKind() == DYNAMIC_PARAM)
			field = newDynamicParamMark(sc.operand(0));
		else
	...
	private static String getNodeName(SqlNode node) {
		switch (node.getKind()) {
		case IDENTIFIER:
		...
		case DYNAMIC_PARAM:
			return newDynamicParamMark(node);
		default:
