...
public abstract class SparkPumpImpl<V, P extends SparkPumpImpl<V, P>> extends Namedly implements Pump<V>, Serializable {
	private static final long serialVersionUID = 5209205358314389930L;
...
//	protected final String name;
	...
	protected SparkPumpImpl(String name, int parallelism) {
		super(name);
		this.name = name;
		if (parallelism < 0)
			this.parallelism = (int) Math.floor(Math.sqrt(Exeter.of().parallelism())) - parallelism;
	...
	protected final void pumping(Supplier<Boolean> sourceEmpty, Runnable pumping) {
		Runnable rr = pumping::run;
		Runnable r = Runnable
				.exception(rr,
						(Consumer<Exception> & Serializable) ex -> logger().error("Pump processing failure for ["
								+ ex.toString() + "] at: \n" + Exceptions.getStackTrace(ex)))
				.until((Supplier<Boolean> & Serializable) () -> !opened() || sourceEmpty.get());
	...
	protected boolean isAllDependsOpen() {
		for (AutoCloseable dep : dependencies)
			if (dep instanceof Openable && !((Openable) dep).opened())
				return false;
		return true;
	}
