...
2. 调用示例
	1. 将sql解析成es查询语句 
		...
		2. sql示例：
			~~~sql
				select * from index..type where zjhm='574646121111111' and name like '*tom*'; 
			~~~
	...
	2. 返回结果
		~~~java
		List<Map<String, Object>> resultMapList = ((com.hzcominfo.albatis.search.es.result.ElasticResult) resultSet).getResultMapList();			
		~~~
3. ES地址
